...
function draw() {
    background(0);
    const nb = params.numBars, //number of bars
        ph = floor(PANEL/(2*nb)), //floor  of the pixel size / twice the number of bars
        f  = frameCount, // ???
        rf = max(1, round(params.staticRate)); // refresh rate of static, minimum of 1

    if (hGap.length !== nb) {
    regenAll(); lastRegen = f; // ??? regenerate all?
